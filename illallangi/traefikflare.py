...
def TraefikFlare(log_level,
    ...
    while True:
        try:
            zones = {zone['name']: zone['id'] for zone in cloudflare.zones.get(params={'per_page': 100})}
            logger.info(f'Cloudflare connection OK, {len(zones)} zones found:')
            for zone in zones:
                logger.info(f'  {zones[zone]}: {zone}')
        except CloudFlareAPIError as e:
            logger.error(f'CloudFlareAPIError getting zones for {cloudflare_email}: {e}')
            continue
    ...
    while True:
        ...
        if current != previous:
            logger.info(f'Hosts changed, {len(current)} hosts found ({len(hosts) - len(current)} filtered):')
            previous = current
            for host in current:
                zone_name = [zone for zone in zones if host.endswith(zone)][0]
                zone_id = zones[zone_name]
                logger.debug(f'Starting {host} {current[host]} in zone {zone_id} {zone_name}')

                try:
                    dns_records = cloudflare.zones.dns_records.get(zone_id, params={'name': host, 'match': 'all', 'type': 'A'})
                except CloudFlareAPIError as e:
                    logger.error(f'CloudFlareAPIError getting dns records for {zone_id}: {host}: {e}')
                    previous = None
                    continue
                except Exception as e:
                    logger.error(f'Exception getting dns records for {zone_id}: {host}: {e}')
                    previous = None
                    continue
                if len(dns_records) > 1:
                    logger.error(f'Received {len(dns_records)} dns records for {zone_id}: {host}, expected 1')
                    previous = None
                    continue
                ...
                if len(dns_records) == 0:
                    ...
                    try:
                        ...
                        logger.error(f'CloudFlareAPIError creating dns record for {zone_id}: {host}: {e}')
                        previous = None
                        continue
                    except Exception as e:
                        logger.error(f'Exception creating dns record for {zone_id}: {host}: {e}')
                        previous = None
                        continue
                ...
                if len(dns_records) == 1:
                    ...
                    if current[host] == old_ip_address:
                        logger.info(f'{host} already set to {current[host]}, no change required.')
                        continue
                    ...
                    try:
                        ...
                        logger.error(f'CloudFlareAPIError updating dns record for {zone_id}: {host}: {e}')
                        previous = None
                        continue
                    except Exception as e:
                        logger.error(f'Exception updating dns record for {zone_id}: {host}: {e}')
                        previous = None
                        continue
                    logger.success(f'{host} updated from {old_ip_address} to {current[host]}.')
